...
class ModuleContentListViews(TemplateResponseMixin, View):
...
class CourseListView(TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        # cach 2; su dung cache
        subjects = cache.get('all_subjects')
        if not subjects:
            subjects = Subject.objects.annotate(total_courses=Count('courses'))
            cache.set('all_subjects', subjects)

        # cach 1: lay tu database, k dung nua khi co cache
        #subjects = Subject.objects.annotate(total_courses=Count('courses'))
        # Count('courses') => courses đc lấy theo tên của mối quan hệ giữa
        # chủ đề và khóa học
        ...
        return self.render_to_response({'subjects': subjects, 'subject':
...
class CourseDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['enroll_form'] = CourseEnrollForm(initial={'course':
                                                                self.object})
        return context
